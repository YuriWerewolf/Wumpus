        ...
        if (self.useLogic):
            for i in range(1, self.boardSize + 1):
                for j in range(1, self.boardSize + 1):
                    ...
                    if self.KB.resolution(logic.expr("~P"+loc)) and self.KB.resolution(logic.expr("~W"+loc)):
                        safeCells.add((i, j))

                    # If no smell, and no breeze, then all neighbors are safe locations.
                    no_smell  = self.KB.resolution(logic.expr("~S"+loc))
                    no_breeze = self.KB.resolution(logic.expr("~B"+loc))
        ...
        if (self.useLogic):
            for i in range(1, self.boardSize + 1):
                for j in range(1, self.boardSize + 1):
                    ...
                    # Locations with gold
                    if self.KB.resolution(logic.expr("G"+loc)):
                        goldCells.add((i, j))
        ...
        if (self.useLogic):
            for i in range(1, self.boardSize + 1):
                for j in range(1, self.boardSize + 1):
                    ...
                    if self.KB.resolution(logic.expr("P" + loc)) or self.KB.resolution(logic.expr("W" + loc)):
                        if ((i, j) in riskyCells): riskyCells.remove((i, j))

                    # If no smell and no breeze, then all neighbors are safe.
                    noSmell  = self.KB.resolution(logic.expr("~S" + loc))
                    noBreeze = self.KB.resolution(logic.expr("~B" + loc))
